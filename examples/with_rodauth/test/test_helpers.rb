...
class HookedTestClass < Minitest::Test
  ...
  def clean_test_db!
    tables = [:account_active_session_keys, :account_authentication_audit_logs,
      :account_email_auth_keys, :account_lockouts, :account_login_change_keys,
      :account_login_failures, :account_otp_keys, :account_password_reset_keys,
      :account_recovery_codes, :account_session_keys, :account_verification_keys, :accounts]
...
class CapybaraTestCase < HookedTestClass
  ...
  Capybara.app = With_rodauth.freeze.app
  ...
  def verify_account_mail_index
    Mail::TestMailer.deliveries.map(&:subject).index("with_rodauth - Verify your account")
  end
