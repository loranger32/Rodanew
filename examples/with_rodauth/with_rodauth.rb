...
class With_rodauth < Roda
  ...
  plugin :sessions, key: "with_rodauth_app.session", secret: secret
  plugin :route_csrf
  ...
  plugin :rodauth do
    ...
    login_label "Email"
    if With_rodauth.production?
      domain ENV["DOMAIN"]
    elsif With_rodauth.development?
      domain "localhost"
    elsif With_rodauth.test?
      domain "www.example.com"
    end

    # Email Base
    email_from "with_rodauth@example.com"
    email_subject_prefix "with_rodauth - "
    send_email { |mail| SendEmailInProductionJob.perform_async(mail) } if With_rodauth.production?

    # Login Password Requirements Base
    password_hash_cost(With_rodauth.test? ? 2 : 12)
    ...
    after_create_account do
      mail = Mail.new do
        from "with_rodauth@example.com"
        to ENV["MY_EMAIL"]
        subject "with_rodauth - New User Signed Up"
  ...
  plugin :content_security_policy do |csp|
    ...
    csp.img_src :self, "data:" # data: is required for Bootstrap 5 svg inline icons
    csp.object_src :none
    csp.frame_src :self
  ...
  plugin :assets,
    css: %w[style.css],
    js: {main: "main.js", recovery_codes: "recovery_codes.js", bs_tooltip: "bs_tooltip.js"},
    group_subdirs: false,
    gzip: true,
  ...
  route do |r|
    r.public
    r.assets unless With_rodauth.production?
